...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['search_box'])) {
    ...
    // Redirect to preserve search in URL
...
// Build redirect URL preserving filter + search
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['property_id'], $_POST['action']) && $_POST['action'] === 'approve') {
        $id = $_POST['property_id']; // keep as string
...
$pending_q = $conn->query("SELECT COUNT(*) FROM property WHERE status = 0")->fetchColumn();
$approved_q = $conn->query("SELECT COUNT(*) FROM property WHERE status = 'approved'")->fetchColumn();
?>
...
<section class="listings">
   ...
   <div class="filter-buttons">
      ...
      <form method="POST" style="margin-left: auto; display: flex; gap: 5px;">
         <input type="text" name="search_box" class="search-box" placeholder="Search by name, address, type..." 
      ...
      <?php if ($search): ?>
         <a href="?filter=<?= $filter ?>" class="btn" style="background: #95a5a6;">
            Clear Search
         </a>
      <?php endif; ?>
   </div>
      ...
      <?php if ($stmt->rowCount() > 0): ?>
         ...
         <div class="box">
            <div class="thumb" style="position: relative;">
               <p class="image-count">
                  <i class="far fa-image"></i> <?= $img_count ?>
               </p>
            ...
            <p class="location">
               <i class="fas fa-map-marker-alt"></i> <?= htmlspecialchars($p['address']) ?>
            </p>
            <p class="price">Rs. <?= number_format($p['price']) ?></p>
            <p class="type">
               <strong><?= ucfirst($p['type']) ?></strong> • <?= ucfirst($p['offer']) ?>
            </p>
...
<?php endif; ?>
               ...
               <form method="POST" style="display: inline;">
                  ...
                  <button type="submit" class="btn-sm delete-btn" 
                          onclick="return confirm('Delete this property permanently? All images will be removed.')">
